...
class PerformanceEvaluator(PerformanceEvaluatorProtocol):
    ...
    def __init__(self) -> None:
        # Initialize storage for recorded results and difficulty mappings.
    ...
    def record(self, result: SolveResult, puzzle_id: str, difficulty: DifficultyLevel) -> None:
        # Record a solver's result for a specific puzzle and difficulty.
    ...
    def get_comparison(self, puzzle_id: str) -> ComparisonTable:
        # Return a ComparisonTable for the given puzzle id using recorded results.
    ...
    def get_aggregate(self, difficulty: DifficultyLevel) -> AggregateStats:
        # Compute aggregate statistics across puzzles of the given difficulty.
    ...
    def export_csv(self, filepath: str) -> None:
        # Export recorded results to CSV using the exporter helper.
    ...
    def _compute_rankings(self, results: dict[SolverType, SolveResult]) -> dict[str, list[SolverType]]:
        # Produce rankings of solvers by various metrics from a results dict.
    ...
    def _compute_algorithm_stats(self, results: list[SolveResult]) -> AlgorithmStats:
        # Compute mean/min/max statistics and solve rate for a solver's results.
        if not results:
    ...
    def get_all_results(self) -> dict[str, dict[SolverType, SolveResult]]:
        # Return a shallow copy of all recorded results.
        return dict(self._results)

    def get_difficulty_map(self) -> dict[str, DifficultyLevel]:
        # Return a copy of the puzzle id -> difficulty mapping.
