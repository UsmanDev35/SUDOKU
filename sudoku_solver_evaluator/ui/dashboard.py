"""Comparative analysis dashboard using Rich tables and charts.

Provides Rich-based rendering of performance comparison tables,
aggregate statistics, and visual bar charts for algorithm comparison.
...
# Human-friendly names for solver types
...
# Bar chart characters for visual comparison
_BAR_CHAR = "█"
_BAR_HALF = "▌"
_BAR_MAX_WIDTH = 40
...
def render_comparison_table(
    comparison: ComparisonTable, console: Optional[Console] = None
) -> None:
    """Render a comparison table for all algorithms on a single puzzle.

    Shows time, states explored, backtracks, and optimality rank for each
    algorithm. The best-performing algorithm per metric is highlighted in
    green/bold.

    Args:
        comparison: ComparisonTable with results and rankings for a puzzle.
    ...
    table = Table(
        title=f"Performance Comparison — Puzzle: {comparison.puzzle_id}",
        caption=f"Difficulty: {comparison.difficulty.value.capitalize()}",
        show_header=True,
    ...
    # Determine best values for each metric (lowest is best)
    ...
    # Get optimality rankings (by states_explored)
    ...
    for solver_type, result in results.items():
        ...
        # Format values, highlighting best in green/bold
...
def render_aggregate_table(
    aggregate: AggregateStats, console: Optional[Console] = None
) -> None:
    """Render aggregate statistics (mean/min/max) for all algorithms at a difficulty level.

    Groups results by difficulty and shows mean, min, and max for time,
    states explored, and backtracks. Also includes solve rate.

    Args:
        aggregate: AggregateStats for a difficulty level.
        console: Optional Rich Console instance; creates one if not provided.
    """
    if console is None:
        console = Console()
    ...
    table = Table(
        title=f"Aggregate Statistics — {difficulty_name} ({aggregate.puzzle_count} puzzles)",
        show_header=True,
        header_style="bold cyan",
    )
    ...
    # Find best mean values for highlighting
...
def render_performance_chart(
    ...
    metric: str = "states_explored",
    console: Optional[Console] = None,
) -> None:
    """Render a bar-chart-style comparison using Unicode bar characters.

    Displays a horizontal bar chart comparing a single metric across
    all algorithms. The longest bar fills the maximum width, and others
    are proportional.

    Args:
        results: Mapping of solver type to SolveResult.
        metric: Which metric to chart. One of 'time_ms', 'states_explored',
                'backtracks'. Defaults to 'states_explored'.
        console: Optional Rich Console instance; creates one if not provided.
    """
    if console is None:
        console = Console()
    ...
    panel = Panel(
        chart_content,
        title=f"[bold]{metric_label} Comparison[/bold]",
        border_style="cyan",
    )
...
def render_grouped_by_difficulty(
    comparisons: dict[DifficultyLevel, list[ComparisonTable]],
    console: Optional[Console] = None,
) -> None:
    """Render comparison tables grouped by difficulty level.

    Iterates through difficulty levels and renders each puzzle's
    comparison table under its difficulty heading.

    Args:
        comparisons: Mapping of difficulty level to list of ComparisonTable objects.
        console: Optional Rich Console instance; creates one if not provided.
    """
    if console is None:
        console = Console()
    ...
    for difficulty in DifficultyLevel:
        ...
        console.print()
        console.rule(
            f"[bold magenta]{difficulty.value.capitalize()} Difficulty[/bold magenta]"
        )
...
def render_full_dashboard(
    ...
    aggregates: dict[DifficultyLevel, AggregateStats],
    console: Optional[Console] = None,
) -> None:
    """Render the full comparative analysis dashboard.

    Combines comparison tables grouped by difficulty, aggregate statistics,
    and performance bar charts into a unified dashboard view.

    Args:
        comparisons: List of all ComparisonTable results.
        aggregates: Mapping of difficulty level to AggregateStats.
        console: Optional Rich Console instance; creates one if not provided.
    """
    if console is None:
        console = Console()
    ...
    # Group comparisons by difficulty
    ...
    # Render aggregate statistics for each difficulty level
    console.print()
    ...
    # Render performance bar chart using the last comparison's results
    if comparisons:
        ...
        if last_comparison.results:
            ...
            render_performance_chart(
                last_comparison.results, metric="states_explored", console=console
            )
            render_performance_chart(
                last_comparison.results, metric="time_ms", console=console
            )


def _format_metric(value: int, best_value: int) -> Text:
    """Format a metric value, highlighting if it equals the best value.

    Args:
        value: The metric value to format.
        best_value: The best (lowest) value for this metric.
